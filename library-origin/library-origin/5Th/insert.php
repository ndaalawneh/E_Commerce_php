...
if(isset($_POST["category-name"])){
    ...
    try{
        ...
        if($image){
            ...
            $_POST["category-url_img"] = $location;
    ...
    if(!isset($_POST["user_id"])){
        try{
            ...
            $sql = "INSERT INTO `user` (`user_id`, `name`, `password`, `gender`, `birthday`, `url_image`, `role`, `email`) VALUES (UUID_SHORT(), :name, :password, :gender, :birthday, :url, :role, :email)";
            $statement = $db->prepare($sql);
            if(!$statement->execute(array(
